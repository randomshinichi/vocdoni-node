...
var zeroReq = &api.APIrequest{}

func reset(r *api.APIrequest) {
	*r = *zeroReq
}
func BenchmarkVochainBatchProof(b *testing.B) {
	...
	censusRoot, _, processID := createDistributedCensusAndProcess(b, cl, dvoteServer, keySet, entityID)
	waitProcess(b, cl, processID)
...
func waitProcess(b *testing.B, cl *client.Client, processID []byte) {
...
func createDistributedCensusAndProcess(b *testing.B, cl *client.Client, dvoteServer testcommon.DvoteAPIServer, keySet []*ethereum.SignKeys, entityID []byte) (censusRoot []byte, censusURI string, processID []byte) {
	// create census
